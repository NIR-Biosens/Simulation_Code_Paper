...
def signal_noise(wavelength, QY_F, epsilon_F_peak, c_F, stokes_shift, Int_LED, PD, filter_width, z, T, A_spot):
    ...
    # no data after 1300 nm -> assumption zero absorption
    ...
    def dark_current(T, i_dark, bandgap):
        return i_dark * np.exp(-bandgap/(2*sc.k*T))  # calculated in (A)
    ...
    for ix in range(length):
        ...
        F_Em = gaussian(wavelength, 30, lambda_Em) # FWHM = 30 nm
        
        F_A = gaussian(wavelength, 30, lambda_Ex) # FWHM = 30 nm
        epsilon_F = F_A/np.max(F_A) *  epsilon_F_peak # calculate the spectral molar absorption coeeficient with a gaussian shape
        
        Phi_Em = np.trapz(Phi_Ex * (1 - np.exp(-1 * np.log(10) * epsilon_F * c_F * z)), wavelength) * F_Em * QY_F   # Photons/s
        
        # create a dicroic mirror
        lambda_x = lambda_Ex + stokes_shift / 2 # edge wavelength is in the middle of excitation and emission wavelength
        T_dichroit = np.ones(len(wavelength)) * 1e-3 # OD3 filtering
        T_dichroit[int(lambda_x - wavelength[0]):] = 0.98  # 98 % Transmission
        
        # create a emission bandpass-filter
        T_longpass = np.ones(len(wavelength)) * 1e-6  # OD6 filtering
        T_longpass[int(lambda_Em - filter_width/2 - wavelength[0]):int(lambda_Em +
                                                                      filter_width/2 - wavelength[0])] = 0.98  # 98 % Transmission
        ...
        I_Ex_wl = Phi_Ex_back * (sc.c * sc.h / wavelength) * R_lamdba    # in (A))
        
        # integrate photons to get intensity detected on camera
        I_Em = np.trapz(I_Em_wl, wavelength) # emission current (A)
        
        I_Ex = np.trapz(I_Ex_wl, wavelength) # background excitation current (A)
        
        # print when emission = 990 nm ((6,5)-SWCNT emission peak)
        if lambda_Em == 990:
            print('\n---------------------------------------------------------------------------------------')
            print('\nemission = 990 nm ((6,5)-SWCNT)')
            print('\nemission current: I = {0:2.3f} nA \n'.format(I_Em*1e9))
            
            # efficiency 
            mu = (np.trapz(Phi_Em)/np.trapz(Phi_Ex)).round(6)
...
## Fluorophore with properties similiar to a CNT
...
ax.set_xlim([600, 1675])
ax.set_ylim([48,58])
ax.set_yticks([49,51,53,55,57])
ax2.set_ylim([21,34.5])
...
plt.savefig(r'U:\InGaAs vs Si\Paper\Main Figures\Figure 1\1c.png', dpi=500)
plt.savefig(r'U:\InGaAs vs Si\Paper\Main Figures\Figure 1\1c.svg')
...
ax.set_xlim([575, 1700])

ax.legend()

fig.tight_layout()

plt.savefig(r'U:\InGaAs vs Si\Paper\Main Figures\Figure 1\S3.png', dpi=500)
plt.savefig(r'U:\InGaAs vs Si\Paper\Main Figures\Figure 1\S3.svg')
