...
def signal_noise(wavelength, QY_F, epsilon_F_peak, c_F, stokes_shift, Int_LED, PD, filter_width, z, T, A_spot):
    ...
    path_af = r'Spectral_data/FITC_Abs.txt'
    data = np.genfromtxt(path_af, delimiter='	', skip_header=7)
    ...
    path_af = r'Spectral_data/FITC_Em.txt'
    data = np.genfromtxt(path_af, delimiter='	', skip_header=7)
    ...
    for ix in range(length):
        ...
        # print when emission = 516 nm (FITC)
        if lambda_Em == 516:
            print('\n---------------------------------------------------------------------------------------')
            print('\nemission = 516 nm (FITC)')
            print('\nemission current: I = {0:2.3f} nA \n'.format(I_Em*1e9))
            
            mu = (np.trapz(Phi_Em)/np.trapz(Phi_Ex)).round(7)
...
## Fluorophore with properties similiar to a FITC
QY_F = 0.92  # quantum yield
epsilon_F_peak = 73000  # molar absorption coefficient (1/(M cm)) 
c_F = 1e-9 # fluorophore concentration (mol/L)
stokes_shift = 516-491 #  stokes shift (nm)
...
fig.tight_layout()
plt.savefig(r'U:\InGaAs vs Si\Paper\Main Figures\Figure 1\S4a.png', dpi=500)
plt.savefig(r'U:\InGaAs vs Si\Paper\Main Figures\Figure 1\S4a.svg')
...
fig.tight_layout()
plt.savefig(r'U:\InGaAs vs Si\Paper\Main Figures\Figure 1\S4c.png', dpi=500)
plt.savefig(r'U:\InGaAs vs Si\Paper\Main Figures\Figure 1\S4c.svg')